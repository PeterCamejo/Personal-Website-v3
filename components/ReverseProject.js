...
const ReverseProject = (props) =>{
    ...
    return(
        <Flex className={styles.projectContainer}  direction={{base:"column", md:"row"}} justify={{base:"end", md:"center"}} align="center" mb={"120px"} >
            ...
            <Box as={motion.div} initial={{opacity:0 , x:20}} animate={imageAnimation}  ref={imageRef} boxShadow="base" backgroundColor={useColorModeValue("#FFCF07", '#06fdd8')} _hover={{backgroundColor:"none"}} className={styles.projectImageContainer} >
                <Image src={props.projectImage} height="500px" width="400px" className={styles.projectImage} />
            </Box>
            <Container  textAlign="right" className={styles.projectDetails}>
                ...
                <Text  as={motion.div} initial={{opacity:0 , y:20}} animate={descriptionAnimation}  ref={descriptionRef} textAlign="left" rounded='md' backgroundColor={{base:"transparent" , md: bg}} boxShadow="base" ml={{base:0 , md:-20}} className={styles.projectDescription} fontSize={{base:"18px" , md: "20px"}}>{props.projectDescription}</Text>
                {props.badges.map((badge,index) =>{
                    return (
                        <Badge as={motion.div} initial={{opacity: 0 , y:20}} custom={index+1} animate={badgeAnimation} ref={badgeRef} boxShadow="lg" className="badge" mr={2} backgroundColor={useColorModeValue("#FFCF07" , '#06fdd8')} color="black" float={{base:"left", md:"none"}} mt={{base:2,md:0}} key={index}>{badge}</Badge>
                    )
                })}
                {props.githubLink ? <Link as={motion.a} initial={{opacity:0 , y:20}} animate={descriptionAnimation}  ref={descriptionRef} whileHover={{scale:1.2}} href={props.githubLink} float={{base:"right",md:"left"}} mt={1} isExternal><Icon as={FiGithub} boxSize={6} ></Icon></Link> : null}{ props.demoLink ? <Link href={props.demoLink} as={motion.a} initial={{opacity:0 , y:20}} animate={descriptionAnimation}  ref={descriptionRef} whileHover={{scale:1.2}} float="right" mr={2} isExternal><ExternalLinkIcon boxSize={6}></ExternalLinkIcon></Link> : null}
